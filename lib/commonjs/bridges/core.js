...
exports.CoreEditorActionType = exports.CoreBridge = void 0;
var _base = _interopRequireDefault(require("./base"));
var _AsyncMessages = require("../RichText/AsyncMessages");
var _focusListener = require("../webEditorUtils/focusListener");
...
let CoreEditorActionType = exports.CoreEditorActionType = /*#__PURE__*/function (CoreEditorActionType) {
...
const CoreBridge = exports.CoreBridge = new _base.default({
  forceName: 'coreBridge',
  tiptapExtension: _extensionDocument.default,
  tiptapExtensionDeps: [_extensionParagraph.default, _extensionText.default],
  ...
  onEditorMessage: ({
    ...
    if (type === CoreEditorActionType.SendHTMLToNative) {
      _AsyncMessages.asyncMessages.onMessage(payload.messageId, payload.content);
      return true;
    }
  ...
  extendEditorInstance: (sendBridgeMessage, webviewRef, editorStateRef, _updateEditorState, platform) => {
    return {
      ...
      getHTML: async () => {
        const html = await _AsyncMessages.asyncMessages.sendAsyncMessage({
          type: CoreEditorActionType.GetHTML
        }, sendBridgeMessage);
        return html;
      },
      getText: async () => {
        const text = await _AsyncMessages.asyncMessages.sendAsyncMessage({
          type: CoreEditorActionType.GetText
        }, sendBridgeMessage);
        return text;
      },
      getJSON: async () => {
        const json = await _AsyncMessages.asyncMessages.sendAsyncMessage({
  ...
  extendEditorState: editor => {
    return {
      isFocused: _focusListener.focusListener.isFocused,
      isReady: true,
      selection: {
